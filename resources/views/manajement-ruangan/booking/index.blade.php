...
<script>
    ...
    $('#peminjaman-table').on('click','.action', function(){
        ...
        if(jenis == 'delete'){
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        method  : 'DELETE',
                        url     : `/manajement-ruangan/booking/`+id+ `/delete`,
                        headers : {
        ...
        if(jenis == 'accept'){
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        method  : 'get',
                        url     : `/manajement-ruangan/booking/`+id+ `/accept`,
                        headers : {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                        beforeSend:function()
        ...
        if(jenis == 'disaccept'){
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        method  : 'get',
                        url     : `/manajement-ruangan/booking/`+id+ `/disaccept`,
                        headers : {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                        beforeSend:function()
        ...
        if(jenis == 'edit'){
            $.ajax({
                method  : 'get',
                url     : `/manajement-ruangan/booking/`+id+ `/edit`,
                beforeSend:function()
                        {
                        $('#overlay').fadeIn(300);
                        },
